...
const FeasibilitySection = () => {
  ...
  const displayedReports = showAll ? feasibilityReports : feasibilityReports.slice(0, 9);

  return (
    <section id="feasibilities" className="section-white py-24 lg:py-32 relative overflow-hidden">
      <div className="container mx-auto px-4 lg:px-8" ref={ref}>
        ...
        >
          <span className="inline-block px-4 py-1.5 rounded-full bg-accent/10 text-accent text-sm font-semibold mb-6 uppercase tracking-wide">
            Feasibility Reports & POCs
          </span>
          <h2 className="text-3xl lg:text-5xl font-display font-bold text-foreground mb-6">
            From <span className="text-gradient">Concept</span> to Reality
          </h2>
          <p className="text-muted-foreground text-lg">
            Explore our comprehensive feasibility studies and proof of concepts
          </p>
        </motion.div>
          ...
          >
            ...
            >
              {showAll ? (
                ...
                <>
                  Show More ({feasibilityReports.length - 9} more) <ChevronDown className="w-5 h-5" />
                </>
              )}
            </button>
          </motion.div>
